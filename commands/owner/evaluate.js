...
executed by the bot owner(s) defined in the config/bot/settings.json file. */
    ...
    run: async (bot, message, args) => {
        ...
        try {
          ...
          if (evaluated.length <= 1900) {
            ...
            let txtFile = "./config/bot/eval.txt";
            writeFile(txtFile, evaluated, "utf8", (err) => {
              if (err) {
                console.error(err);
                return message.channel.send(`Sorry **${message.author.username}**, I couldn't write the evaluated code to file!`);
              }
              console.log(evaluated);
              return message.channel.send(`🕑 **${hrDiff[0] > 0 ? `${hrDiff[0]}s ` : ''}${(hrDiff[1] / 1000000).toFixed(3)}ms**\nThe evaluated code is longer than Discord's message limit, but the result is in the console's logs. In case you can't access that now, I made a text file containing the same data.`, { files: [txtFile] } );
            });
          }
        }
        catch (err) {
